...
class Canevas(tk.Canvas):
    ...
    def dessinerRectangleVide(self, x, y, l, h, col):
        return ObjetGraphique(self.create_rectangle(x, y, x + l, y + h, outline=col, width=2), x, y, col)
    ...
    def recupererObjet_single(self, x, y):
    ...
    def recupererObjet(self, x, y):
        ...
        if not ido:
            return None
        objets_graphiques = [ObjetGraphique.annuaire[id_obj] for id_obj in ido]
    ...
    # AJOUT FONCTION
    ###############################################################################
    def changerBG(self, color=str):
        self.configure(bg=color)

    def creerButton(self, texte, fonction, x, y, width=20, height=1, col="black", font=("Times New Roman", 12),
                    col2='white'):
        Bouton = tk.Button(self, text=texte, command=fonction, width=width, height=height, fg=col, font=font, bg=col2)
